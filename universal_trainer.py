...
class UniversalModelTrainer:
    def __init__(self, fee_rate=0.001, target_profit_pct=0.5, lookforward_candles=10,
                 stop_loss_pct=1.5, slippage_pct=0.05, calibrate=False):
        """
        fee_rate: fraction (e.g., 0.001 = 0.1%)
        target_profit_pct: percent e.g., 0.5 means 0.5%
        lookforward_candles: how many candles to look ahead for TP
        stop_loss_pct: stop loss percent (e.g., 1.5 => 1.5%)
        slippage_pct: assumed slippage on execution (percent)
        calibrate: whether to calibrate probabilities after training
        """
...
def train_universal_pipeline(data_file='universal_features.csv',
                             fee_rate=0.001, target_profit=0.5, lookforward=10,
                             stop_loss=1.5, slippage=0.05, calibrate=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--profit', type=float, default=0.5)
    parser.add_argument('--lookforward', type=int, default=10)
    parser.add_argument('--stoploss', type=float, default=1.5)
