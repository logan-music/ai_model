...
class UniversalModelTrainer:
    ...
    # per-coin optimizer (expanded grid)
    ...
    # create labels supporting provided coin_trailing_params to avoid optimization leakage
    def create_universal_labels(self, df, use_trailing=True, coin_trailing_params=None):
        print("🔖 Creating labels per coin (TP/SL + trailing simulation)")
        df = df.copy()
        ...
        if coin_trailing_params:
            ...
            # If no params provided, run small search (not recommended for train/test split safety)
            if use_trailing and self.optimize_trailing:
                ...
                for sym in symbols:
                    ...
                    if len(coin_df) < 50:
                        ...
                        continue
                    params, score = self.optimize_trailing_per_coin(coin_df)
        ...
        for sym in symbols:
            ...
            vol_ok = (vol_series > 0.8).astype(int)
            vol_ok = vol_ok.reindex(coin_df.index).fillna(0).astype(int)
            ...
            if len(future_returns) != n or len(trailing_returns) != n or len(labels) != n or len(vol_ok) != n:
                raise ValueError(f"Index length mismatch for {sym}: {len(future_returns)},{len(trailing_returns)},{len(labels)},{len(vol_ok)},{n}")

            df.loc[coin_df.index, 'future_return'] = pd.Series(future_returns, index=coin_df.index)
            df.loc[coin_df.index, 'trailing_return'] = pd.Series(trailing_returns, index=coin_df.index)
            df.loc[coin_df.index, 'target'] = pd.Series(labels, index=coin_df.index)
            df.loc[coin_df.index, 'signal_volume_ok'] = vol_ok.values

            buys = int(np.nansum(labels))
...
# ---------------------------
# Utility: decision threshold optimizer (leakage-free, validation/holdout)
# ---------------------------
def optimize_decision_threshold(model, scaler, df, feature_cols, test_size=0.2, min_trades=20, holdout_frac=0.5):
    ...
    Returns: best_threshold, stats_dict { 'validation': {...}, 'holdout': {...} }
    """
    ...
    for sym in df['symbol'].unique():
        ...
        # validation = earlier portion, holdout = last hold_len rows
        if hold_len >= len(test_part):
            # if holdout would take everything, leave at least 1 row for validation
    ...
    for thr in thrs:
        ...
        # improved metric prefers positive PnL and reasonable trade count
    ...
    # fallback: if no threshold met min_trades, pick threshold maximizing avg_return * count on validation
    if best_score == -np.inf:
        ...
        if fallback_best == -np.inf:
            # final fallback -> choose median-ish threshold
            best_thr = 0.8
            best_stats = {}

    # compute holdout stats for reporting (untouched during selection)
    hold_stats = {}
    if not hold_df.empty:
    ...
    return float(best_thr), {'validation': best_stats, 'holdout': hold_stats}
...
def train_universal_pipeline(data_file='universal_features.csv',
    ...
    if model is not None:
        ...
        trainer.save_universal_model('universal_scalp', threshold=thr)

        # Save feature list and trailing params + threshold metadata for inference
        ...
        with open('universal_threshold.pkl', 'wb') as f:
            pickle.dump({'threshold': float(thr), 'stats': stats}, f)
