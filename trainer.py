...
class UniversalModelTrainer:
    ...
    def create_universal_labels(self, df):
        ...
        df = df.sort_values(['symbol', df.index.name])
        ...
        for sym in symbols:
            ...
            # Use reindex + fillna to guarantee same index and length
            ...
            if len(future_returns) != n or len(labels) != n or len(vol_ok) != n:
                raise ValueError(f"Index/length mismatch for {sym}: future_returns={len(future_returns)} labels={len(labels)} vol_ok={len(vol_ok)} coin_rows={n}")

            # assign back to df using Series with coin_df.index to guarantee alignment
            ...
            # vol_ok is already a Series aligned to coin_df.index
            df.loc[coin_df.index, 'signal_volume_ok'] = vol_ok

            print(f"  {sym}: labeled {int(np.nansum(labels))} / {n} as BUY ({(int(np.nansum(labels))/n*100):.2f}%)")
...
def train_universal_pipeline(data_file='universal_features.csv',
    ...
    # ----------------------------
    # DATA CLEANING (fix inf/large/NaN issues)
    # ----------------------------
    if feature_cols:
        ...
        # Clip extremes to avoid overflow in downstream (choose reasonable cap)
        try:
            df[feature_cols] = df[feature_cols].clip(-1e6, 1e6)
        except Exception:
            # fallback: elementwise clipping
            for c in feature_cols:
                df[c] = np.clip(df[c].astype(float), -1e6, 1e6)

        # Drop rows missing any required features or target
        before = len(df)
    ...
    # run cross-coin validation (optional informative)
