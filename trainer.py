...
class UniversalModelTrainer:
    ...
    def save_universal_model(self, model_prefix='universal_scalp', threshold=0.5,
        ...
        Also create legacy copies for older pipelines/zip expectations:
          - universal_model.pkl  (copy of universal_scalp_model.pkl)
          - universal_threshold.pkl (copy of universal_scalp_threshold.pkl)
          - universal_trailing_params.pkl (copy of universal_scalp_trailing_params.pkl)
        """
        ...
        try:
            ...
            # --- Create legacy/alias copies so older code / zip contains expected names ---
            # copy model alias: universal_model.pkl (older pipeline expects this)
            try:
                shutil.copyfile(model_file, "universal_model.pkl")
                print("Copied model_file -> universal_model.pkl")
            except Exception as e:
                print("Warning: failed to copy model_file -> universal_model.pkl:", e)

            # copy trailing params alias
            try:
                shutil.copyfile(trailing_file, "universal_trailing_params.pkl")
                print("Copied trailing_file -> universal_trailing_params.pkl")
            except Exception as e:
                print("Warning: failed to copy trailing_file -> universal_trailing_params.pkl:", e)

            # copy threshold alias
            try:
                shutil.copyfile(threshold_file, "universal_threshold.pkl")
                print("Copied threshold_file -> universal_threshold.pkl")
            except Exception as e:
                print("Warning: failed to copy threshold_file -> universal_threshold.pkl:", e)
...
def train_universal_pipeline(data_file='universal_features.csv',
    ...
    if model is not None:
        ...
        ok = trainer.save_universal_model('universal_scalp',
                                    threshold=thr,
                                    min_prob=DEFAULT_MIN_PROB,
                                    prob_edge=DEFAULT_PROB_EDGE,
                                    top_pct=DEFAULT_TOP_PCT,
        ...
        try:
            ...
            # also ensure a legacy feature file name for compatibility
            try:
                shutil.copyfile('universal_features.pkl', 'universal_features_list.pkl')
            except Exception:
                pass
        except Exception as e:
            print("Warning: failed to save universal_features.pkl:", e)

        # Also ensure the trailing params and threshold are available under legacy names if not already copied
        if not os.path.exists('universal_trailing_params.pkl') and os.path.exists('universal_scalp_trailing_params.pkl'):
            try:
                shutil.copyfile('universal_scalp_trailing_params.pkl', 'universal_trailing_params.pkl')
            except Exception:
                pass
        if not os.path.exists('universal_threshold.pkl') and os.path.exists('universal_scalp_threshold.pkl'):
            try:
                shutil.copyfile('universal_scalp_threshold.pkl', 'universal_threshold.pkl')
            except Exception:
                pass
