...
# =========================================================
DEFAULT_MIN_PROB      = 0.68   # Threshold ya msingi
DEFAULT_PROB_EDGE     = 0.20
...
TEST_COINS            = []        # Tupu = tumia temporal split (bora zaidi)
TRAIN_RATIO           = 0.75     # 75% ya data kwa training

# Labeling
STRONG_SIGNAL_MULT    = 2.5     # Precision-focused labeling
MIN_NET_RETURN        = 0.004   # 0.4% minimum net return ili label=1
LOOKFORWARD_CANDLES   = 16      # FIX: 16 candles = 80 dakika kwa 5m data (ilikuwa 6=30min)

# Coins za kuondoa - zina avg_net hasi (hazifai kwa training)
# Hizi zilipatikana kutoka matokeo ya trailing optimization
EXCLUDE_COINS = {
    'BTCUSDT',    # avg_net=-0.00061 (hasi)
    'BNBUSDT',    # avg_net=-0.00053 (hasi)
    'FORTHUSDT',  # avg_net=-0.00052 (hasi)
    'RUNEUSDT',   # avg_net=-0.00033 (hasi)
    'ALPINEUSDT', # avg_net=-0.00022 (hasi)
    'ETHUSDT',    # avg_net=-0.00011 (karibu sifuri - precision 0%)
}

# Feature selection
MAX_FEATURES          = 40      # Top features tu (reduce overfitting)
MIN_MUTUAL_INFO       = 0.001   # Minimum mutual info kwa feature
CORR_THRESHOLD        = 0.92    # Maximum correlation kati ya features

# Threshold optimization
MIN_PRECISION_FOR_THR = 0.35    # FIX: Punguza kidogo (0.40→0.35) kwa data hii
...
# =========================================================
# =========================================================
# WEIGHTED ENSEMBLE - Top-level class (lazima iwe hapa ili
# joblib/pickle iweze kuihifadhi vizuri - local class haiwezi pickle)
# =========================================================
class WeightedEnsemble:
    """
    Ensemble ya RF + GB kwa weighted probability averaging.
    Imewekwa top-level (si ndani ya function) ili iweze
    kuhifadhiwa na joblib.dump() bila hitilafu.
    """
...
def train_precision_ensemble(X_train, y_train, calibrate: bool = False):
    ...
    - GB na oversampling sahihi (si VotingClassifier)
    - WeightedEnsemble top-level -> pickle inafanya kazi
    """
    ...
    # GB iliyoboreshwa na oversampling ya BUY class
    ...
    # Oversample BUY class kwa GB (manual balancing)
    if n_pos > 0 and ratio > 1:
        pos_idx    = np.where(y_train == 1)[0]
        neg_idx    = np.where(y_train == 0)[0]
        target_n   = min(n_pos * 3, len(neg_idx))
        pos_sample = np.random.choice(pos_idx, size=target_n, replace=True)
        neg_sample = np.random.choice(neg_idx, size=target_n, replace=False)
    ...
    # Rudisha top-level WeightedEnsemble (pickle-safe)
    ...
    if results_table:
        ...
        for _, row in results_df.head(5).iterrows():
            print(f"    thr={row['threshold']:.2f} | prec={row['precision']:.3f} | "
                  f"trades={int(row['trades']):3d} | avg_ret={row['avg_ret']:.4f} | score={row['score']:.4f}")
...
class UniversalModelTrainerV2:
    ...
    def prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Onesha coins zitakazotumika
        all_coins      = df['symbol'].unique()
        excluded       = [c for c in all_coins if c in EXCLUDE_COINS]
        active_coins   = [c for c in all_coins if c not in EXCLUDE_COINS]
        if excluded:
            print(f"  ⏭️  Coins zilizoondolewa (avg_net hasi): {excluded}")
        print(f"  ✅ Coins zinazofunzwa: {active_coins}")

        parts = []
        for sym in all_coins:
            # Ruka coins zenye avg_net hasi
            if sym in EXCLUDE_COINS:
                continue
            cd = df[df['symbol'] == sym].sort_index().copy()
            if len(cd) < 50:
                print(f"  ⚠️ {sym}: rows chache sana ({len(cd)}), skipping")
                continue
            try:
...
# Backward compatibility alias - pipeline.py inaweza kutumia jina lolote
train_universal_pipeline = train_pipeline
